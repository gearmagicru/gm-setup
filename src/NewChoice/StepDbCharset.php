...
class StepDbCharset extends SetupStep
{
     ...
     * {@inheritdoc}
     */
    public string $viewName = 'new/dbcharset';
    ...
    {
        return $this->installer->isCompleted('new:db');
    }
    ...
    protected function getAdapter(string $charset = null, string $collate = null): Adapter
    {
        ...
        // если база данных уже создана на сервере
    ...
    protected function runTest(): bool
    {
        ...
        // если база данных уже создана на сервере
        $dbCreated = $this->state->dbCreated ?? false;
        if (!$dbCreated) {
            try {
                $command = $adapter->createCommand()
                    ->createDatabase($this->state->dbSchema, $this->state->dbCharset, $this->state->dbCollate);
                $command->query();
            } catch (CommandException $exception) {
                $this->addError($exception->error, $this->t('SQL query execution error'));
